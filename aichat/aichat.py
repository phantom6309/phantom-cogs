...
class AIChat(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        default_global = {
            "model_url": None,  # URL to download model from
            "model_loaded": False,
            "max_length": 50,
            "temperature": 0.7,
            "response_chance": 0.05,  # 5% chance for random responses
        }
        
        default_guild = {
            "enabled_channels": [],
            "prefix_responses": True,  # Whether to mention user in response
    ...
    async def _load_model_if_exists(self):
        """Load model if it exists locally"""
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    inputs,
                    max_new_tokens=await self.config.max_length(),
    ...
    async def setup_model(self, ctx, model_url: str = None):
        ...
        Provide a direct download URL to your trained model zip file.
        """
    ...
    async def toggle_channel(self, ctx, channel: discord.TextChannel = None):
        """Enable/disable AI responses in a channel"""
    ...
    async def chat_command(self, ctx, *, message: str):
        ...
        Example: `[p]chat Hello, how are you?`
        """
        if not await self.config.model_loaded():
    ...
    async def on_message(self, message):
        """Listen for messages and potentially respond"""
        ...
        # Check if bot was mentioned or random response
...
def setup(bot):
    if not TRANSFORMERS_AVAILABLE:
        raise RuntimeError("The transformers library is required for this cog. Install it with `pip install transformers torch`")
