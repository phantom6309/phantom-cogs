...
class CrabRave(commands.Cog):
    ...
    @commands.max_concurrency(2, commands.BucketType.default)
    @checks.bot_has_permissions(attach_files=True)
    async def mary(self, ctx: commands.Context, *, text: str) -> None:
        """Make mary rave videos
        ...
        async with ctx.typing():
            t = ctx.message.clean_content[len(f"{ctx.prefix}{ctx.invoked_with}") :]
            t = t.upper().replace(", ", ",").split(",")
            if not await self.check_video_file(MARY_LINK, "mary_template.mp4"):
                return await ctx.send("I couldn't download the template file.")
            if not await self.check_font_file():
                return await ctx.send("I couldn't download the font file.")
            if len(t) != 2:
            ...
            if (not t[0] and not t[0].strip()) or (not t[1] and not t[1].strip()):
                return await ctx.send("Cannot render empty text")
            fake_task = functools.partial(self.make_mary, t=t, u_id=ctx.message.id)
            task = self.bot.loop.run_in_executor(None, fake_task)

            try:
                await asyncio.wait_for(task, timeout=300)
            except asyncio.TimeoutError:
                # log.error("Error generating maryrave video", exc_info=True)
                await ctx.send("maryrave Video took too long to generate.")
                return
            fp = cog_data_path(self) / f"{ctx.message.id}maryrave.mp4"
            file = discord.File(str(fp), filename="maryrave.mp4")
            try:
                await ctx.send(files=[file])
            except Exception:
                log.error("Error sending mikurave video", exc_info=True)
    ...
    def make_mary(self, t: str, u_id: int) -> bool:
        """Non blocking miku rave video generation from DankMemer bot

        https://github.com/DankMemer/meme-server/blob/master/endpoints/crab.py
        """
        fp = str(cog_data_path(self) / f"Verdana.ttf")
        clip = VideoFileClip(str(cog_data_path(self)) + "/mary_template.mp4")
        # clip.volume(1.0)
        text = TextClip(t[0], fontsize=48, color="DarkSlateGrey", font=fp)
        text2 = (
            TextClip(
        ...
        video = video.volumex(0.7)
        video.write_videofile(
            str(cog_data_path(self)) + f"/{u_id}maryrave.mp4",
            threads=1,
            preset="superfast",
            verbose=False,
            logger=None,
            temp_audiofile=str(cog_data_path(self) / f"{u_id}maryraveaudio.mp3")
            # ffmpeg_params=["-filter:a", "volume=0.5"]
        )
        clip.close()
        video.close()
    ...
    @commands.command(aliases=["dwrave"])
    @commands.cooldown(1, 20, commands.BucketType.guild)
    @commands.max_concurrency(2, commands.BucketType.default)
    @checks.bot_has_permissions(attach_files=True)
    async def dw(self, ctx: commands.Context, *, text: str) -> None:
        """Make dw rave videos
        ...
        async with ctx.typing():
            t = ctx.message.clean_content[len(f"{ctx.prefix}{ctx.invoked_with}") :]
            t = t.upper().replace(", ", ",").split(",")
            if not await self.check_video_file(DW_LINK, "dw_template.mp4"):
                return await ctx.send("I couldn't download the template file.")
            if not await self.check_font_file():
                return await ctx.send("I couldn't download the font file.")
            if len(t) != 2:
            ...
            if (not t[0] and not t[0].strip()) or (not t[1] and not t[1].strip()):
                return await ctx.send("Cannot render empty text")
            fake_task = functools.partial(self.make_dw, t=t, u_id=ctx.message.id)
            task = self.bot.loop.run_in_executor(None, fake_task)

            try:
                await asyncio.wait_for(task, timeout=300)
            except asyncio.TimeoutError:
                # log.error("Error generating dwrave video", exc_info=True)
                await ctx.send("dwrave Video took too long to generate.")
                return
            fp = cog_data_path(self) / f"{ctx.message.id}dwrave.mp4"
            file = discord.File(str(fp), filename="dwrave.mp4")
            try:
                await ctx.send(files=[file])
            except Exception:
                log.error("Error sending mikurave video", exc_info=True)
    ...
    def make_dw(self, t: str, u_id: int) -> bool:
        """Non blocking miku rave video generation from DankMemer bot

        https://github.com/DankMemer/meme-server/blob/master/endpoints/crab.py
        """
        fp = str(cog_data_path(self) / f"Verdana.ttf")
        clip = VideoFileClip(str(cog_data_path(self)) + "/dw_template.mp4")
        # clip.volume(1.0)
        text = TextClip(t[0], fontsize=48, color="DarkSlateGrey", font=fp)
        text2 = (
            TextClip(
        ...
        video = video.volumex(0.7)
        video.write_videofile(
            str(cog_data_path(self)) + f"/{u_id}dwrave.mp4",
            threads=1,
            preset="superfast",
            verbose=False,
            logger=None,
